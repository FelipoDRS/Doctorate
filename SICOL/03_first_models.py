...
for i in y_var_da:
    ...
    scores = cross_validate(clf,X_da.loc[boll1,:] , y2mod.loc[boll1], cv=cv,scoring=['r2','neg_mean_squared_error'])
...
for i in y_var_dp:
    ...
    scores = cross_validate(clf,X_dp.loc[bool3,:] , y2mod.loc[bool3], 
...
for i in y_var_dp:
    y2mod=dados_dp.loc[:,i]
    bool1=np.logical_and(y2mod<y2mod.mean()+4*y2mod.std(),
                         y2mod>y2mod.mean()-4*y2mod.std())
    bool2=np.logical_and(np.logical_not(np.isnan(y2mod)).values,
                           np.logical_not(np.sum(np.isnan(X_dp),axis=1)>=1).values)
    bool3=np.logical_and(bool1,bool2)
...
for i in y_var_da:
    y2mod=dados_da.loc[:,i]
    boll1=np.logical_and(y2mod<y2mod.mean()+4*y2mod.std(),y2mod>y2mod.mean()-4*y2mod.std())
    scores = cross_validate(clf,X_da2.loc[boll1,:] , y2mod.loc[boll1], cv=cv,scoring=['r2','neg_mean_squared_error'])
    scores_collect.append([i,scores])
    print(i,np.mean(scores['test_r2']),np.median(np.sqrt(-scores['test_neg_mean_squared_error'])))
...
#X_dp2['RSO2']=1/(X_dp2['RSO_despar'])
X_dp2['RSO2']=np.log(X_dp2['RSO_despar'])
...
for i in y_var_dp:
    ...
    bool1=np.logical_and(y2mod<y2mod.mean()+4*y2mod.std(),y2mod>y2mod.mean()-4*y2mod.std())
    bool2=np.logical_and(np.logical_not(np.isnan(y2mod)).values,
                           np.logical_not(np.sum(np.logical_or(
                                   np.isnan(X_dp2),np.isinf(X_dp2)),axis=1)>=1).values)
    bool3=np.logical_and(bool1,bool2)
    scores = cross_validate(clf,X_dp2.loc[bool3,:] , np.log(y2mod.loc[bool3]), cv=cv,scoring=['r2','neg_mean_squared_error'])
    scores_collect.append([i,scores])
    print(i,np.mean(scores['test_r2']),np.mean(np.sqrt(-scores['test_neg_mean_squared_error'])))
