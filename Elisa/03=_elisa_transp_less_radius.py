...
circles = cv2.HoughCircles(gray2*7, cv2.HOUGH_GRADIENT, 1.2, 70)
...
for i in range(circles_num.shape[0]):
    ...
    intensity[i,2]=np.nanmedian(collect_int)
...
DF_dados.plot(x='sat',y='absorb',style='+')
DF_dados['log absorb']=np.log(DF_dados['absorb'])
print(DF_dados.corr())
DF_dados.plot(x='sat',y='log absorb',style='+')
