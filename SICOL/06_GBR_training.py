...
GBR=GradientBoostingRegressor( learning_rate=0.03, n_estimators=300, subsample=0.9, 
...
for i in y_var_dp:
    ...
    scores = cross_validate(GBR,X_dp.loc[bool3,dict_feature[i]] , 
                             y2mod.loc[bool3], cv=cv,scoring=['r2','neg_mean_squared_error'])
    scores_collect.append([i,scores])
    print(i,np.mean(scores['test_r2']),np.median(np.sqrt(-scores['test_neg_mean_squared_error'])))
    GBR.fit(X_dp.loc[bool3,dict_feature[i]] , y2mod.loc[bool3])
#    print(GBR.score(X_dp.loc[bool3,dict_feature[i]] , y2mod.loc[bool3]))
#    print(np.vstack((dict_feature[i],GBR.feature_importances_)).T)
