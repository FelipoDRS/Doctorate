...
def check_min_max_nan(X_prime,X_test,var_int,flag_both=False):
    ...
    if flag_both:
        return X_test.loc[flag_sample,:],X_test.loc[np.logical_not(flag_sample),:],(flag_sample,flag_max,flag_min)
    else:
...
for i in y_var_dp:
    ...
    GBR.fit(X2mod,y2mod)
    # Novos dados 1
    dado_novo,dado_storage=check_min_max_nan(dado_inicial,dado_novo1,x_var_dp)
    dado_rej_collect=pd.concat((dado_rej_collect,dado_storage))
#    print('perda de dados min_max',
      #    dado_novo.shape[0]-dado_novo1.shape[0])
    dado_novo,dado_storage=check_covar(Mean,Cov,dado_novo.loc[:,x_var_dp])
    dado_rej_collect=pd.concat((dado_rej_collect,dado_storage))
 ...
 #   print('perda de dados cov',
    ...
    GBR.fit(X2mod,y2mod)
    dado_novo,dado_storage=check_min_max_nan(dado_2,dado_novo2,x_var_dp)
    dado_rej_collect=pd.concat((dado_rej_collect,dado_storage))
#    print('perda de dados min_max retreino2',
  #        dado_novo.shape[0]-dado_novo2.shape[0])
    dado_novo,dado_storage=check_covar(Mean,Cov,dado_novo2.loc[:,x_var_dp])
    dado_rej_collect=pd.concat((dado_rej_collect,dado_storage))
...
#    print('perda de dados cov retreino2',
    ...
    dado_rej_collect_list.append(dado_rej_collect.copy())
...
for i in y_var_da:
    ...
    GBR.fit(X2mod,y2mod)
    # Novos dados 1
    dado_novo,dado_storage=check_min_max_nan(dado_inicial,dado_novo1,x_var_da2)
    dado_rej_collect=pd.concat((dado_rej_collect,dado_storage))
  #  print('perda de dados min_max',
     #     dado_novo.shape[0]-dado_novo1.shape[0])
    dado_novo,dado_storage=check_covar(Mean,Cov,dado_novo.loc[:,x_var_da])
    dado_rej_collect=pd.concat((dado_rej_collect,dado_storage))
  ...
  #  print('perda de dados cov',
   #       dado_novo.shape[0]-dado_novo1.shape[0])
    ...
    # Novos dados 2
    dado_2=pd.concat((dado_inicial_limpo,pd.concat((dado_novo,y2mod1),axis=1)))  
    X2mod=dado_2.loc[:,x_var_da].reset_index(drop=True)

    Cov=X2mod.cov().loc[x_var_da,x_var_da]
    Mean=X2mod.mean().loc[x_var_da]
    y2mod=dado_2.loc[:,i].reset_index(drop=True)
    GBR.fit(X2mod,y2mod)
    dado_novo,dado_storage=check_min_max_nan(dado_2,dado_novo2,x_var_da2)
    dado_rej_collect=pd.concat((dado_rej_collect,dado_storage))
  #  print('perda de dados min_max retreino2',
     #     dado_novo.shape[0]-dado_novo2.shape[0])
    dado_novo,dado_storage=check_covar(Mean,Cov,dado_novo2.loc[:,x_var_da])
    dado_rej_collect=pd.concat((dado_rej_collect,dado_storage))
